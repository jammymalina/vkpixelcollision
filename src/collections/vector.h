...
static inline size_t vector_min_(size_t a, size_t b) {
    return a < b ? a : b;
}

static inline size_t vector_max_(size_t a, size_t b) {
    return a > b ? a : b;
}

static inline size_t vector_between_(size_t x, size_t min, size_t max) {
    return vector_max_(min, vector_min_(max, x));
}

static inline size_t vector_enforce_size_t_(size_t value) {
    return value;
}
...
#define vector_realloc_(pv, new_cap) (                                         \
    (pv)->data = vector_reallocdata_((pv)->data, new_cap,                      \
...
#define vector_resize_(pv, new_cap) (                                          \
    (pv)->cap == (new_cap) || (                                                \
        (new_cap) > 0 ?                                                        \
            vector_realloc_(pv, new_cap) : (vector_clear(pv), true)            \
    )                                                                          \
)
...
#define vector_reserve(pv, min_cap)                                            \
    vector_reserve_internal_(pv,                                               \
        vector_max_(min_cap, VECTOR_MINCAP_))

#define vector_reserve_internal_(pv, min_cap) ((min_cap) <= (pv)->cap || (     \
    (min_cap) <= vector_max_cap_(pv) &&                                        \
    vector_realloc_(pv,                                                        \
        vector_between_(vector_growsize_((pv)->cap), min_cap,                  \
        vector_max_cap_(pv))))                                                 \
)

#define vector_shrink_to_fit(pv) (void)                                        \
    vector_resize_(pv, (pv)->size)
...
#define vector_push_all(pv, items, count)                                      \
    vector_push_all_internal_(pv, items, vector_enforce_size_t_(count))
...
#define vector_insert_hole(pv, index, count)                                   \
    vector_insert_hole_internal_(pv, vector_enforce_size_t_(index),            \
        vector_enforce_size_t_(count))
...
#define vector_insert(pv, index, item) (                                       \
    vector_insert_hole(pv, index, 1) && (                                      \
        (pv)->data[index] = (item),                                            \
        true                                                                   \
    )                                                                          \
)
...
static inline void vector_rotate_array_left_(char* array, size_t len,
    size_t distance)
{
...
static inline void vector_rotate_array_right_(char* array, size_t len,
    size_t distance)
{
    vector_rotate_array_left_(array, len, len - distance);
}

static inline void vector_move_(char* array, size_t index, size_t count,
    size_t target)
{
    if (index < target) {
        vector_rotate_array_left_(&array[index], target - index + count,
            count);
    } else {
        vector_rotate_array_right_(&array[target], index - target + count,
            count);
    }
}

#define vector_move_slice(pv, index, count, target)                            \
    vector_move_slice_internal_(pv,                                            \
        vector_enforce_size_t_(index),                                         \
        vector_enforce_size_t_(count),                                         \
        vector_enforce_size_t_(target)                                         \
    )
...
#define vector_move(pv, index, target)                                         \
    vector_move_slice(pv, index, 1, target)

#define vector_remove_slice_noshrink(pv, index, count)                         \
    vector_remove_slice_noshrink_internal_(pv,                                 \
        vector_enforce_size_t_(index),                                         \
        vector_enforce_size_t_(count)                                          \
    )
...
#define vector_remove_noshrink(pv, index)                                      \
    vector_remove_slice_noshrink(pv, index, 1)
...
#define vector_swap_remove(pv, index)                                          \
...
#define vector_index_of(pv, item, pidx)                                        \
    do {                                                                       \
        ...
        for (vector_find_idx_ = 0; vector_find_idx_ < (pv)->size &&            \
            ((pv)->data[vector_find_idx_] != (item));                          \
            ++vector_find_idx_);                                               \
...
#define vector_foreach(item, pv)                                               \
    for (size_t vector_idx_##item = 0;                                         \
        vector_idx_##item < (pv)->size &&                                      \
            ((item) = (pv)->data[vector_idx_##item], true);                    \
         ++vector_idx_##item)
